...
| `GET /api/events?handoff=<ligne>` | connecté | `{ csi, handoffs: [{ kind?, peer, toEmail, toName, byEmail, byName, at }] }` — passages d'appel en cours sur la ligne (écrits par `POST /api/events` à chaque `transfer` visant une adresse, fichier `keyyo/handoff/<csi>.json`) **plus les annonces d'appel** qui visent la personne connectée (`kind: 'dial'`, écrites à chaque `dial` portant `toEmail`, fichier `keyyo/handoff/to/<empreinte>.json`, valables 90 s) : sa fenêtre d'appel nomme le collègue qui l'appelle et montre sa photo, alors que le numéro qui sonne est celui du site. Pas de route dédiée : **Vercel Hobby limite un déploiement à 12 fonctions `api/*.js`, toutes prises** — une 13ᵉ fait échouer le déploiement |
| `GET /api/photo` | connecté | `?u=<adresse>&s=<48…360>` → `image/jpeg` (cache privé 1 j), `404` sans photo, `503` si Graph refuse. Photo Entra lue par `api/_graph.js` avec le jeton d'**application** (permission `User.ReadBasic.All`, consentement admin), gardée dans le Blob `keyyo/photos/v2/<empreinte>/<taille>.jpg`. L'adresse est essayée comme nom d'utilisateur, puis la personne est cherchée par adresse de messagerie (`mail`, `proxyAddresses`) ; le `404` porte `X-Photo-Reason: no-photo` (mémorisé 7 j) ou `no-user` (1 j) |
